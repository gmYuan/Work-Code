...
var server = http.createServer(function(request, response){
  ...
  if(path === '/'){
    ...
    if (request.headers.cookie) {
      cookies = request.headers.cookie.split("; ")  //存在多个cookie时
    }
    															
    if (cookies){
    	cookies.forEach(cookie => {
      	let [key,value] = cookie.split("=")
      	hash[key] = value   // {sessID: XXX, ....}
    	})
    }

    // 读取sesssionId相关联的 用户信息
    let sessionInfo = sessions[hash.sessionID]
    let email
    if (sessionInfo) {
    	email = sessionInfo["sign-in-email"]
    }
    ...
    readBody(request).then( (data) => {
      ...
      if (iscorrect) {
        ...
        // 1 cookie中 存一个随机数sessionId + 把 sessionId:用户信息 存入session中
        let sessionID = Math.random() * 100000
        sessions[sessionID] = {"sign-in-email": email}
    ...
    let result= fs.readFileSync('./js/main.js','utf8');
    response.statusCode = 200
    ...
    let result= fs.readFileSync('./css/default.css','utf8')
    mdheader = md5(result)
    response.setHeader('ETag', `${mdheader}`)

    // console.log(request.headers)
    if(request.headers["if-none-match"] === mdheader) {
